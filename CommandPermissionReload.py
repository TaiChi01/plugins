...
PLUGIN_METADATA = {
	'id': 'commandpermissionreload',
	'version': '1.0',
	'name': 'CommandPermissionReload',
	'description': '!!<command>, 以MCDR可控制的权限系统执行指令',
	'author': 'DrLee-lihr',
	'link': 'https://github.com/DrLee-lihr/CommandPermissionReload'
}
...
def on_load(server, old):
	server.register_help_message('!!<command>', '以MCDR可控制的权限系统执行指令§7<command>§r')
...
def on_user_info(server,info):
    ...
    if info.content.startswith("!!advancements") and server.get_permission_level(info)>=commandpermission_advancements:
        server.execute("execute as "+info.player+" at "+info.player+" run "+info.content.lstrip('!!'))
        server.tell(info.player,"§7<command>§r执行成功")
